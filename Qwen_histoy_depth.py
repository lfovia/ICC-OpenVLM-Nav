...
-------------------------------------
Zero-shot Object Navigation using Qwen-VL reasoning with short history context and depth modality

Pipeline:
1. Habitat environment (MP3D or HM3D)
2. Qwen-VL for visual reasoning (zero-shot) with short rolling history and depth input
...
def qwen_reason_action(image, depth,goal, step_num=0, history=None):
    """
    ...
    caption = qwen_generate_scene_caption(image, depth, goal)
    ...
    if history:
        ...
        history_str = "RECENT SCENE HISTORY:\n" + history_str + "\n"
...
def compute_average_time(episode_times):
    """Compute and return average time per episode."""
    if not episode_times:
        return 0.0
    return sum(episode_times) / len(episode_times)
...
def run_zero_shot(eval_episodes=10):
    ...
    for ep in tqdm(range(eval_episodes), desc="Episodes"):
        ...
        os.makedirs(f"./results_qwen_hisdepthv2/{goal}", exist_ok=True)
        dir_out = f"./results_qwen_hisdepthv2/{goal}/traj_{ep}"
        ...
        while not env.episode_over and t < 100:
            ...
            act, caption = qwen_reason_action(img,depth, goal, step_num=t, history=history)
        ...
        episode_end_time = time_module.time()
        ...
        # Save metrics
        if metrics_all:
            with open("qwen_metrics_history_depthv2.csv", "w", newline="") as f:
                writer = csv.DictWriter(f, fieldnames=metrics_all[0].keys())
                writer.writeheader()
                writer.writerows(metrics_all)

        print(f"[EP {ep}] Done. Episode time: {episode_duration:.2f}s")

    env.close()
    avg_time = compute_average_time(episode_times)
    print(f"[INFO] All episodes completed. Average time per episode: {avg_time:.2f}s")
