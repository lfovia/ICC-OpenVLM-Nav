...
-------------------------------------
Zero-shot Object Navigation using Qwen-VL reasoning.

Pipeline:
1. Habitat environment (MP3D or HM3D)
2. Qwen-VL for visual reasoning (zero-shot)
3. DINO for open-set object detection
4. SAM for segmentation
...
def qwen_generate_caption(image, goal):
    ...
    prompt = (
        ...
        "Mention any doors, doorways, hallways, corridors, alleyways, furniture, objects, walls, or open spaces. "
...
def qwen_reason_action(image, goal, step_num=0):
    """Two-stage reasoning: generate caption first, then decide action based on caption + image."""
    
    # Stage 1: Generate detailed scene caption
    caption = qwen_generate_caption(image, goal)
    
    # Stage 2: Use caption + image + goal to decide action
    img_pil = save_image_for_qwen(image)
    ...
    prompt = (
        ...
        f"Reply with exactly ONE WORD: FORWARD, LEFT, RIGHT, or STOP"
    )
...
def run_zero_shot(eval_episodes=10):
    """Run Qwen zero-shot navigation episodes."""
    ...
    for ep in tqdm(range(eval_episodes), desc="Episodes"):
        ...
        # obs already contains the initial observation from env.reset()

        # Navigation loop - simple per-step reasoning from current view only
        # Stuck detection: track image hashes to detect repetitive views
        import hashlib
        stuck_history = []
        ...
        while not env.episode_over and t < 100:
            ...
            # Ask Qwen for action from current view
            act = qwen_reason_action(img, goal, step_num=t)
        ...
        # Save metrics
        with open("qwen_objnav_metrics_50.csv", "w", newline="") as f:
